...
class LazadaUIController extends Controller
{
    ...
    public function refreshToken()
    {
        try
        {
            ...
            if($updatedToken) {
                Log::channel('lazada.refresh_token')->info('New tokens generated. Please reload the page.');
    ...
    public function updatePrice()
    {
        ...
        if(count($finalErrorList) > 0){
            Log::channel('lazada.item_master')->error("Update Price - ".count($finalErrorList)." SKUs have issues while updating the price: "."\n".implode("\n",$finalErrorList));
        }
    ...
    public function updateStock()
    {
        ...
        if(count($finalErrorList) > 0){
            Log::channel('lazada.item_master')->error("Update Stock - ".count($finalErrorList)." SKUs have issues while updating the stock: "."\n".implode("\n",$finalErrorList));
        }
    ...
    public function generateSalesOrder()
    {
        ...
        $sellerVoucher = $odataClient->getOdataClient()->from('U_MPS_ECOMMERCE')->where('Code','SELLER_VOUCHER')->first();
        $shippingFee = $odataClient->getOdataClient()->from('U_MPS_ECOMMERCE')->where('Code','SHIPPING_FEE')->first();
